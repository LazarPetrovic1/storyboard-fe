import { useCallback, useMemo, useState } from "react";
import { connect, useDispatch, useSelector } from "react-redux";
import { useNavigate, useParams } from "react-router-dom";
import { alerts } from "../actions";
import { storyTypes } from "../actions/types";
import { MarkdownEditor, RadioButton } from "../components";
import { BoxShadowButton } from "../styled";

function EditStory({ setAlert }) {
  const { id } = useParams();
  const storyItem = useSelector(state => state.stories.stories[parseInt(id)]);
  const [story, setStory] = useState(() => storyItem);
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const onChange = e => setStory(() => ({ ...story, [e.target.name]: e.target.value }));
  const onSubmit = e => {
    e.preventDefault();
    if (story.title && story.snippet && story.fullStory && story.type) {
      dispatch({ type: storyTypes.UPDATE_STORY_FE, payload: { id, story } });
      setAlert("Story updated successfully", "success");
      navigate('/stories');
    } else {
      setAlert("All fields are required", "danger");
    }
  };
  const onFullStoryChange = useCallback((e) => setStory((prevStory) => ({ ...prevStory, fullStory: e })), []);
  const onRadioChange = val => setStory(() => ({ ...story, type: val }));
  const hasStoryChanged = useMemo(() => (
    (storyItem.fullStory !== story.fullStory) ||
    (storyItem.title !== story.title) ||
    (storyItem.type !== story.type) ||
    (storyItem.snippet !== story.snippet)
    // eslint-disable-next-line
  ), [story.fullStory, story.title, story.type, story.snippet]);
  return (
    <article className="centralize">
      <form className="border border-1 px-5 py-4 container-sm" onSubmit={onSubmit}>
        <h2 className="display-4 text-center">Change the story</h2>
        <div className="mb-3">
          <label htmlFor="title" className="form-label">Title</label>
          <input
            className="form-control"
            id="title"
            name="title"
            value={story.title}
            onChange={onChange}
          />
        </div>
        <div className="mb-3">
          <label htmlFor="snippet" className="form-label">Snippet</label>
          <input
            className="form-control"
            id="snippet"
            name="snippet"
            aria-describedby="snippetHelper"
            value={story.snippet}
            onChange={onChange}
          />
          <small id="snippetHelper">This part is read to the players by the narrator.</small>
        </div>
        <div className="mb-3">
          <label htmlFor="fullStory">Full story</label>
          <MarkdownEditor
            placeholder="Enter the entire story here."
            value={story.fullStory}
            onChange={onFullStoryChange}
          />
          <small id="fullStoryHelper">This part is only known to the narrator.</small>
        </div>
        <div >
          <label htmlFor="type" className="mb-2">Type of case</label>
          <div className="mb-3 d-flex justify-content-between">
            <RadioButton storyType={story.type} name="type" id="suicide" onChange={() => onRadioChange("suicide")} />
            <RadioButton storyType={story.type} name="type" id="accident" onChange={() => onRadioChange("accident")} />
            <RadioButton storyType={story.type} name="type" id="homicide" onChange={() => onRadioChange("homicide")} />
          </div>
        </div>
        {hasStoryChanged && (
          <div className="d-flex justify-content-end my-4">
            <BoxShadowButton className="offset">
              <i className="fa-solid fa-paper-plane" />&nbsp;&nbsp;Submit
            </BoxShadowButton>
          </div>
        )}
      </form>
    </article>
  )
}

export default connect(null, { setAlert: alerts.setAlert })(EditStory)
